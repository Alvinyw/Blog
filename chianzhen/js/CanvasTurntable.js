...
(function ($) {
    $.fn.AddCanvasTurntable = function (options) {
        var defaults = {
			...
			colors: '', /* 插件用到的颜色，应设置成数组形式 */
			rotateTime: '', /* 转盘旋转一次的时间，单位 s，默认是 6s */
			alertText: '', /* 全部选中之后的弹框内容 */
			resultTitle: '', /* result 的标题描述 */
			fontSize: '', /* 扇形的字体大小，默认是 16px */
			defaultSpacing: '' /* 扇形文本离转盘外边框的距离，默认是 35 */
		...
		//扇形的字体大小，默认是 16px
		var fontSize = settings.fontSize?settings.fontSize:16;
		
		//扇形文本离转盘外边框的距离，默认是 35
		var defaultSpacing = settings.defaultSpacing?settings.defaultSpacing:35;
		...
		var idSuffix = Date.parse(new Date())/1000,
			canvasWrapperID = thisID + "canvasWrapper-"+ idSuffix,
			canvas1ID = thisID +  "canvas1-"+ idSuffix,
			canvas2ID = thisID +  "canvas2-"+ idSuffix,
			clickBtnID = thisID +  "clickBtn-"+ idSuffix,
			resultID = thisID +  "result-"+ idSuffix;
		...
		function initCanvasNode(){
			...
			var canvasZhuanPan = $("<canvas id='"+canvas1ID+"' class='canvas1' width='" + canvasWidth +"px' height='" + canvasWidth +"px'>");
			...
			var canvasZhiZhen = $("<canvas id='"+canvas2ID+"' class='canvas2' width='" + canvasWidth +"px' height='" + canvasWidth +"px'>");
			canvasZhiZhen.css({"position":"absolute","top":"0","left":"0","z-index":"30"});
			canvasWrapper.append(canvasZhiZhen);
			
			var clickBtn = $("<button id='"+clickBtnID+"' class='startBtn'>");
			...
			var resultDiv = $("<div id='"+resultID+"' class='result' width='" + canvasWidth +"px'>");
		...
		function initTurntable(){
			...
			function initSectorText(){	 
				...
				for ( var i = 0; i < textNum; i++) {
					...
					ctx1.rotate(i*step);
					ctx1.font = "Bold "+ fontSize +"px Microsoft YaHei";
					ctx1.fillStyle = colors[3];					
					var arrText = sectorText[i].split('');
					for(var j=0;j<arrText.length;j++){
						if(arrText[j]=='('||arrText[j]=='（'){
							arrText[j] = '︵';
						}else if(arrText[j]==')'||arrText[j]=='）'){
							arrText[j] = '︶';
						}
						//-8 是为了让文字向左偏移半个字的宽度（大概是 8px）
						ctx1.fillText(arrText[j], -8, (fontSize*20/16)*j-(canvasWidth/2-defaultSpacing), 30);
					}
					ctx1.closePath();
					ctx1.restore();
				}
			...
			function initPoint(){ 
				...
				ctx2.closePath();
				//小圆文字
				ctx2.font = "Bold 18px Microsoft YaHei"; 
